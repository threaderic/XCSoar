...
NavigatorRenderer::DrawTaskText(
  ...
  const PixelRect &rc,
  [[maybe_unused]] const NavigatorLook &look_nav,
  ...
  Font font;
  // TODO: replace this ugly ratio_dpi by a better implementation. through NavigatorLook?
  // see DialogLook.cpp / InfoBoxLook.cpp
  // this magic number allows to scale correctly all texts even if with custom dpi
  ...
  canvas.Select(font);
  UnitSymbolRenderer::Draw(canvas, unit_p, unit, look_infobox.unit_fraction_pen);
  ...
  font.Load(FontDescription(Layout::VptScale(font_height_waypoint*ratio_dpi)));
  canvas.Select(font);
  ...
  if (canvas.GetWidth() > canvas.GetHeight() * 3.6)
    font.Load(FontDescription(Layout::VptScale(rc_height*ratio_dpi * 32 / 200)));
  else
    font.Load(FontDescription(Layout::VptScale(rc_height*ratio_dpi * 24 / 200)));
...
NavigatorRenderer::DrawProgressTask(
  ...
  const NavigatorLook &look_nav, const TaskLook &look_task) noexcept {
  const int rc_height = rc.GetHeight();
  const int rc_width = rc.GetWidth();
  ...
  const Pen pen_f(Layout::ScalePenWidth(1), look_nav.inverse ? COLOR_WHITE : COLOR_BLACK);
  const Pen pen_fi(Layout::ScalePenWidth(1), look_nav.inverse ? COLOR_BLACK : COLOR_WHITE);
  canvas.Select(pen_f);
